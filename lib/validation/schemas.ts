...
export const createScenarioSchema = z.object({
  input_text: z.string().min(1, 'Input text is required').max(5000, 'Input text must be 5000 characters or less'),
...
export const retestSchema = z.object({
  ...
  improvementNote: z.string().max(1000, 'Improvement note must be 1000 characters or less').optional(),
});
...
export const createRatingSchema = z.object({
  stars: z.number().int().min(1, 'Rating must be between 1 and 5').max(5, 'Rating must be between 1 and 5'),
  feedback_text: z.string().max(2000, 'Feedback must be 2000 characters or less').optional(),
