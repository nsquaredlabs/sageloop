...
describe("POST /api/projects - Validation", () => {
  ...
  it("should accept valid project creation request", async () => {
    ...
    // Temperature should NOT be present (removed for consistent outputs)
    expect(validBody.model_config).not.toHaveProperty("temperature");
  });
});

describe("Model Config Validation", () => {
  it("should not require temperature (removed for consistent outputs)", () => {
    // Temperature is no longer used - model configs should work without it
    const modelConfig = {
      model: "gpt-4",
      system_prompt: "You are a helpful assistant",
    };
    ...
    expect(modelConfig).not.toHaveProperty("temperature");
  });
