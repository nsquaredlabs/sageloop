...
export async function GET(_request: Request, { params }: RouteParams) {
  try {
    ...
    const { data: configured, error: checkError } = await supabase
      .rpc('check_workbench_api_keys', { workbench_uuid: workbenchId });
...
export async function POST(request: Request, { params }: RouteParams) {
  try {
    ...
    const existingKeysObj = (existingKeys as Record<string, string> | null) || {};
...
export async function DELETE(request: Request, { params }: RouteParams) {
  try {
    ...
    return NextResponse.json({
      success: true,
      configured: configured || {},
      message: `${provider === 'openai' ? 'OpenAI' : 'Anthropic'} API key removed. Projects will now use the free tier model (GPT-3.5 Turbo).`,
    });
  } catch (error) {
    console.error('API error:', error);
    return NextResponse.json(
